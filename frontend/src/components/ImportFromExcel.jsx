    ...
    reader.onload = (e) => {
      try{
        ...
        const parsedStudents = jsonData.map((row) => {
          ...
          student.levels = {
            vocabulary: new Levels(Number(row.Vocabulary_initial), Number(row.Vocabulary_target), Number(row.Vocabulary_final)),
            pronunciation: new Levels(Number(row.Pronunciation_initial), Number(row.Pronunciation_target), Number(row.Pronunciation_final)),
            grammar: new Levels(Number(row.Grammar_initial), Number(row.Grammar_target), Number(row.Grammar_final)),
            listening: new Levels(Number(row.Listening_initial), Number(row.Listening_target), Number(row.Listening_final)),
            conversation: new Levels(Number(row.Conversation_initial), Number(row.Conversation_target), Number(row.Conversation_final)),
          };
          return student;
        });
