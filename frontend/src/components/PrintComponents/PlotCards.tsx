...
const PlotCards: React.FC<PlotCardsProps> = ({ levels, language }) => {
  ...
  return (
    <>
      <div id="info-card" className="grid-col-1 grid gap-3">
        {categories.map((item) => {
          return (
            <div key={item} className="border border-slate-700">
              <div
                id="info-card-title"
                className="h-[24px] bg-[rgba(0,161,174,.7)]"
                key={`${item}-title`}
              >
              ...
              <div key={`${item}-description`} className="p-[2px]">
                <RenderLevel category={item} studentLevel={finalValue[item]} language={language} />
              </div>
            </div>
          );
        })}
