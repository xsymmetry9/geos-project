...
const text = (phrase: SPRStringKey, language: Language): string =>
  labelText[language]["SPR"][phrase];
...
export const AttendanceInfo: React.FC<AttendanceInfoProps> = ({
  ...
  return (
    <div id="student-info" className="mr-[3rem] justify-self-end">
      ...
      <p className={`capitalize ${fontStyle()}`}>
        {text("total_lessons", language)}:{" "}
        {totalLessons != 0 ? `${totalLessons} ${text("times", language)}` : "NA"}
      </p>
      <p className={`capitalize ${fontStyle()}`}>
...
export const Table: React.FC<TableProps> = ({ levels, language }) => {
  ...
  const Row = ({ label }: { label: LevelKey }) => {
    ...
    return (
      <tr className="even:bg-white-50 border-r border-l border-slate-600 last:border-b-1 odd:bg-[rgba(0,161,173,.2)]">
        <td className="p-[2px] text-center capitalize">{text(label, language)}</td>
        {labels.map((skill, idx) => (
          <td key={idx} className="p-[2px] text-center">
            {levels[skill][label]}
          </td>
        ))}
        <td className="p-[2px] text-center">{sum()}</td>
        <td className="p-[2px] text-center">{avg()}</td>
      </tr>
    );
  };
  ...
  return (
    <>
      <table className="table-levels m-auto mt-1 w-full table-fixed border-collapse text-[12px]">
        <Legend language={language} />
        <thead className="border-t border-r border-l border-slate-500">
          <tr>
            {headers.map((item, index) => (
              <th
                key={index}
                className="h-[24px] bg-[rgba(0,161,174,.7)] text-slate-800 capitalize"
              >
        ...
        <tbody>
          {["initial", "target", "final"].map((item) => (
            <Row key={item as LevelKey} levels={levels} label={item as LevelKey} />
          ))}
        </tbody>
      </table>
    </>
...
export const Comment: React.FC<CommentProps> = ({ comment, language }) => {
  ...
  return (
    <div id="feedback" className="h-[240px] border border-slate-700">
      <div className="h-[24px] bg-[rgba(0,161,174,.7)]">
        <p className={`ml-2 font-bold text-slate-800 ${fontStyleTitleComment()}`}>
          {text("comment", language)}
        </p>
      </div>
      <div className="h-[210px] overflow-hidden px-2 py-1">
...
export const Signature: React.FC<{ language: Language }> = ({ language }) => {
  return (
    <div className="signature-section mt-3">
      <p className="text-xs/6 capitalize">{text("signature", language)}</p>
