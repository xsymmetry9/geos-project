...
const LanguageAspect: React.FC<LanguageAspectProps> = ({
  inputData,
  aspectName,
  handleLevelInput,
  language,
      ...
      <>
        <div className="flex w-full justify-between">
          ...
          {!displayHelp[itemName] && (
            <button className="cursor-pointer" id={`${itemName}-open`} onClick={displayHandlerOpen}>
        ...
        <label htmlFor={`${aspectName}-${itemName}`}>
          ...
          >
            ...
            <option className="font-secondary text-md text-base" value="select_score">
              Select Score
            </option>

            {/* Reads and maps all level from JSON file */}
            {getAllLevelsInformationByAspect({ name: aspectName, lang: language }).map(
              (item_Value, index) => (
                <option
  ...
  return (
    <>
      <div className="p-t-3">
        <h2 className="border-dark-green my-3 border-0 border-b-2 text-lg font-bold capitalize">
          {labelText[language].SPR[aspectName]}
        </h2>
        <div className="grid grid-cols-1 gap-3">
          {(["initial", "target", "final"] as LevelKey[]).map((item, index) => (
            <div key={index}>
              <PlotSelectOptionLevel
                numIndex={index}
                itemName={item}
                aspectName={aspectName}
                key={item}
